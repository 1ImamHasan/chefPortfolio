...
const SKILL_ICONS = { UtensilsCrossed, Flame, Award, ChefHat }
  ...
  return (
    <div ref={ref}>
      <div className="flex justify-between mb-1">
        <span className="text-xs font-sans font-bold uppercase tracking-widest text-gray-400">{name}</span>
        <span className="text-xs font-bold text-brand-gold">{level}%</span>
      </div>
    ...
    <section id="about" className="py-16 md:py-24 bg-[#1f1f1f]">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
          ...
          <motion.div className="w-full md:w-1/2 relative group"
            initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <div className="absolute inset-0 border-2 border-brand-gold transform translate-x-4 translate-y-4 transition-transform duration-300 group-hover:translate-x-2 group-hover:translate-y-2 hidden md:block" />
            <img src="/photosImam/resume.jpg" alt={`${name} Chef Profile`} loading="lazy"
          ...
          <motion.div className="w-full md:w-1/2"
            ...
            <h2 className="text-4xl md:text-5xl font-sans font-bold text-white mb-4">The Chef's Table</h2>
            <div className="h-0.5 w-16 bg-brand-gold mb-8" />
            <p className="text-gray-400 leading-relaxed mb-8 text-lg font-light">
              Hello, I'm <span className="text-brand-gold font-bold">{name}</span>. As a dedicated Trainee Chef,
              I focus on the intersection of classic tradition and modern innovation. My journey is defined by a
              relentless pursuit of flavor balance, precise technique, and the art of plating.
            </p>
            <div className="space-y-4 mb-8">
              {progressSkills.map((s) => <SkillBar key={s.name} name={s.name} level={s.level} />)}
            </div>
