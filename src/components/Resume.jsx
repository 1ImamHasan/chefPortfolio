      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }}
            ...
            <h3 className="text-xl md:text-2xl font-serif italic text-white mb-10">Professional Journey</h3>
            {timeline.map((item) => (
              <div key={item.id} className="border-l-2 border-brand-gold/30 pl-8 pb-10 relative">
                <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full ${
                  item.active ? 'bg-brand-gold shadow-[0_0_20px_rgba(217,119,6,0.8)]' : 'bg-[#2C2C2C] border-2 border-brand-gold'
                }`} />
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }}
            ...
            <Swiper modules={[Pagination, Autoplay]} slidesPerView={1} spaceBetween={30}
              loop={true} grabCursor={true} pagination={{ clickable: true }}
